...
function build(context) {    
    ...
    typesAsArray.forEach(function(el) {
        ...
        linkData = collectLinks(el.functions || [], linkData, prefixer(urlPrefix + '/' + kind + '/' + el.name + '/#func-'), el.name + '.')
        linkData = collectLinks(el.properties || [], linkData, prefixer(urlPrefix + '/' + kind + '/' + el.name + '/#prop-'), el.name + '.')
        linkData = collectLinks(el.cases || [], linkData, prefixer(urlPrefix + '/' + kind + '/' + el.name + '/#cases'), el.name + '.')
        // linkData = collectLinks(el.aliases || [], linkData, prefixer(urlPrefix + '/' + kind + '/' + el.name + '/#aliases'), el.name + '.')

        if (el.imp) {
            for (var impKey in el.imp) {
                var imp = el.imp[impKey];
                linkData = collectLinks(imp.functions || [], linkData, prefixer(urlPrefix + '/' + kind + '/' + el.name + '/#func-'), el.name + '.')
                linkData = collectLinks(imp.properties || [], linkData, prefixer(urlPrefix + '/' + kind + '/' + el.name + '/#prop-'), el.name + '.')
                linkData = collectLinks(imp.cases || [], linkData, prefixer(urlPrefix + '/' + kind + '/' + el.name + '/#cases'), el.name + '.')
                // linkData = collectLinks(imp.aliases || [], linkData, prefixer(urlPrefix + '/' + kind + '/' + el.name + '/#aliases'), el.name + '.')
            }
        }
    ...
    linkData = collectLinks(context.functions, linkData, prefixer(urlPrefix + '/func/'));
