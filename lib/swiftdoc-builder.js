...
function customizeGraph(svgData) {
    return svgData.replace(/(<g[^>]+>)<title>.+<\/title>/, '$1')      // fix title
                  .replace(/<polygon fill=\"white\"[^>]+>/, '')       // clear background
...
function build(context) {    
    ...
    typesAsArray.forEach(function(el) {
        ...
        if (hasHierarchy)
            outputGraph('' + kind + '/' + el.name + '/hierarchy', dot);
    })
