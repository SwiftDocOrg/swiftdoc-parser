...
function convertComment(value) {
    value = value.replace(/\\\\ ?&#039;/g, '\'')             // weird characters
                ...
                // Swift 2 special notations
                
                .replace(/- parameter +([^ ,]+):/g,          // parameter
                         '\r**`$1`:** ')     
                .replace(/- SeeAlso: (.+)/gi,                 // see also
                        '**See Also:** <span class="see_also">$1</span>')     
...
function customizeGraph(svgData) {
    return svgData.replace(/<title>[^<]+<\/title>/g, '')                   // get rid of titles
                  ...
                  .replace(/\/(type|protocol)\//, urlPrefix + '/$1/')          // add root url 
...
function build(context) {    
    ...
    var linkData = collectLinks(context.operators, { }, function (item) { return urlPrefix + '/operator/' + item.slug; });
