...
function inferTypeAliases(context, type) {
    if (checkConformance(context, type, 'CollectionType') && (context.types[type].kind != 'protocol')) {
        var collection = context.types[type];
        ...
        collection.matchedAliases = {};
        
        // check if they already have type aliases
        context.types[type].aliases.forEach(function(alias) {
            switch (alias.name) {
                case 'Element':
                    element = alias.type;
                    collection.matchedAliases.element = alias.type;
                    break;
                case 'Index':
                    index = alias.type;
                    collection.matchedAliases.index = alias.type;
                    break;
                case 'SubSequence':
                    subsequence = alias.type;
                    collection.matchedAliases.subsequence = alias.type;
                    break;
                case 'Generator':
                    generator = alias.type;
                    collection.matchedAliases.generator = alias.type;
                    break;
            }
        });
        
        // check the subscripts for index, subsequence, and element
        for (var i = 0; i < collection.subscripts.length; i++) {
            if (collection.subscripts[i].params[0].type.match(/\bRange\b/)) {
                if (subsequence == '')
                    subsequence = collection.subscripts[i].ret.line.replace(/ *\{[^\}]+\}/, '').trim();
            } else {
                if ((index == '') || (collection.subscripts[i].params[0].type == index) || (collection.subscripts[i].params[0].type == 'Index')) {
                    if (element == '')
                        element = collection.subscripts[i].ret.line.replace(/ *\{[^\}]+\}/, '').trim();
                }
                
                if (index == '')
                    index = collection.subscripts[i].params[0].type;
            }
        }
        ...
        if (subsequence == '') {
            ...
            if (found.type) {
                subsequence = found.type.replace('<Self>', '<' + type + collection.generic.formatted + '>');
            }
        }
        
        // search for generator
        if (generator == '') {
            var matchedFunction = collection.functions.filter(function (f) {
            ...
            if (matchedFunction.length > 0) {
                ...
                if (found.type) {
                    generator = found.type.replace('<Self>', '<' + type + collection.generic.formatted + '>');
                }
            }
        }
        ...
        var addAlias = function(aliasKey, aliasName, aliasType) {
            if (!collection.matchedAliases[aliasKey]) {
                collection.matchedAliases[aliasKey] = aliasType;
