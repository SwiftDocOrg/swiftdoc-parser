...
function satisfiesWhereClause(context, type, whereClause) {
    ...
    if (whereClause == 'SubSequence : SequenceType, SubSequence.Generator.Element == Generator.Element, SubSequence.SubSequence == SubSequence') return kWhereClausePass;
        ...
        if (matches = clauseGroups[i].match(sameTypeRE)) {
            ...
            if (left == 'element') {
                if (context.types[type].matchedAliases) {
                    ...
                    if (thisElement.indexOf('(') == 0) { return kWhereClauseFail; }
...
function attachDefaultImplementations(context, type, protocols) {
    ...
    if (DEBUG_EXTENSIONS) { console.log('#####################################\n' + type ); }
...
function inferGenerator(context, type) {
    ...
    if (!generator) return;
    addFoundAlias(context, type, 'generator', API.Iterator, generator);
}
...
function inferAssociatedTypes(context, type) {
    ...
    if (isSequence) {
        ...
        console.log(type + ' (gen): ' + local.matchedAliases.generator);
        console.log(type + ' (ele): ' + local.matchedAliases.element);
        console.log(type + ' (ind): ' + local.matchedAliases.index);
        console.log(type + ' (sub): ' + local.matchedAliases.subsequence);
    }
}
...
function inferTypeAliases(context, type) {
    ...
    if (isSequence) {
        ...
        // Element information is elided for UTF8View and UTF16View
        if (type == 'String.UTF8View') {
            local.aliases.push( { kind: 'typealias', name: 'Element', type: 'UInt8', proto: '', 
                    comment: '*Type alias inferred.* Also known as `CodeUnit`.' } );
        } else if (type == 'String.UTF16View') {
            local.aliases.push( { kind: 'typealias', name: 'Element', type: 'UInt16', proto: '', 
                    comment: '*Type alias inferred.* Also known as `CodeUnit`.' } );
        }
