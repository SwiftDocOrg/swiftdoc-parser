...
function buildStdLibDOT(types) {
    // build an array of tuples (2-element arrays, here) with 
    // start and endpoints for paths of inheritance
    var paths = Object.keys(types).reduce(function(p, typeName) { 
//         var inheritsTuples = types[typeName].inherits.map(function(el) { return [el, typeName] });
        var inheritedTuples = types[typeName].inherited.map(function(el) { return [typeName, el] });
        return p.extend(inheritedTuples);
    }, []);

    // map tuples onto DOT edge syntax 
    // e.g., "_CollectionType" -> "CollectionType"
    var dotPaths = paths.map(function(el) {
        var nameFrom = (types[el[0]]) ? types[el[0]].name : el[0];
        var nameTo   = (types[el[1]]) ? types[el[1]].name : el[1];
        return '    "' + nameFrom + '" -> "' + nameTo + '"';
    });
    ...
    var typeNames = paths.reduce(function(p, c) { return p.extend(c); }, [])
                         ...
                         .filter(function(el) { return (types[el] != null); })
                         .filter(function(el) { return (types[el].kind != 'protocol'); })
                         .map(function (el) { return types[el].name;
                         });
    ...
    var dotNodes = paths.reduce(function(p, c) { return p.extend(c); }, [])
                        ...
                        .map(function(el) {
                            ...
                            return '    "' + types[el].name + 
                                    '" [URL="/type/' + el + '/", style=filled]';
                        });
    
    return dotPrefixForStdLib() + 
           dotNodes.join('\n') + 
           dotPaths.join('\n') + 
           dotPostfixWithTypes(typeNames);
}
...
Grapher.prototype.getStdLibDOT = function(context) {
    return buildStdLibDOT(context.types);
}
